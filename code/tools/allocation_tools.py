...
def allocate_functional_requirements(tool_input, cat):
    ...
    Input: "all FSRs" or specific FSR ID
    """
    ...
    if not fsrs:
        return "Error: No FSRs found. Please derive FSRs first with: derive FSRs for all goals"
    
    try:
        # Import allocator
        from ..generators.allocation_generator import AllocationGenerator
        
        # Perform allocation
        allocator = AllocationGenerator(llm_function=cat.llm)
        allocation_result = allocator.allocate_fsrs(fsrs, system_name)
        
        # Update FSRs with allocation
        for fsr in fsrs:
            if fsr['id'] in allocation_result:
                fsr['allocated_to'] = allocation_result[fsr['id']]['element']
                fsr['element_type'] = allocation_result[fsr['id']]['type']
        
        # Store updated FSRs
        cat.working_memory['fsc_functional_requirements'] = fsrs
        cat.working_memory['fsc_allocation_matrix'] = allocation_result
        ...
        output = f"Successfully allocated {len(fsrs)} FSRs to architectural elements for {system_name}.\n\n"
        output += "Allocation follows ISO 26262-3:2018, Clause 7.4.2.8 requirements.\n\n"
        
        # List allocations in simple format
        for fsr in fsrs:
            output += f"{fsr['id']}: {fsr['description'][:80]}...\n"
            output += f"  Allocated to: {fsr.get('allocated_to', 'TBD')}\n"
            output += f"  Element type: {fsr.get('element_type', 'Not specified')}\n"
            output += f"  ASIL: {fsr['asil']}\n"
            output += "\n"
        
        log.info(f"✅ Allocated {len(fsrs)} FSRs")
